# Graph Report - Thesis  (2026-08-04)

## Corpus Check
- 107 files · ~291,307 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 905 nodes · 1500 edges · 56 communities (55 shown, 1 thin omitted)
- Extraction: 96% EXTRACTED · 4% INFERRED · 0% AMBIGUOUS · INFERRED: 66 edges (avg confidence: 0.74)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `bf7bf5b7`
...
5. `smooth_surface()` - 16 edges
6. `_smooth_surface_core()` - 16 edges
7. `main()` - 15 edges
8. `merge_planar_surface()` - 14 edges
9. `render_screenshot()` - 13 edges
10. `main()` - 12 edges
...
- `main()` --calls--> `WebcamSource`  [INFERRED]
  DriveView/drive_view.py → EmissivityCalculation/emissivity/sources.py
- `main()` --calls--> `ZedUvcSource`  [INFERRED]
  DriveView/drive_view.py → EmissivityCalculation/emissivity/sources.py
...
Cohesion: 0.10
Nodes (43): _any_perpendicular(), _axis_aligned_basis(), _axis_target(), _drop_small_components(), fill_enclosed_cells(), _footprint_cells(), _greedy_rectangles(), _label_components() (+35 more)

### Community 3 - "main"
Cohesion: 0.08
Nodes (31): load_scalar_or_map(), main(), parse_args(), Radiometric calibration of the thermal camera (draft).  Corrects the apparent-te, Interpret a CLI argument as a float, or else as a map file path., show_maps(), stats(), Atmospheric transmission in the LWIR band.  Implements the standard single-band (+23 more)
...
Cohesion: 0.10
Nodes (34): main(), print_info(), class_name(), _color_lut(), colorize(), ndarray, TUM-FACADE semantic classes: id -> name and id -> RGB color.  Class ids and name, (MAX_CLASS_ID+1, 3) float array mapping class id -> RGB. (+26 more)
...
Cohesion: 0.15
Nodes (21): ClassMergeStats, MergeSummary, Provenance for one output rectangle, parallel to a SubSurface's `polygons`., Per-class diagnostic counts from one merge_planar_surface() call., Diagnostics for one merge_planar_surface() call, one entry per class., RectMergeInfo, cube_origin(), _grid_from_index() (+13 more)
...
Cohesion: 0.14
Nodes (10): ABC, FrameSource, ImageSource, _open_capture(), Path, Frame sources: still image, webcam, and ZED 2i stereo camera.  All sources retur, ZED 2i single-eye RGB frames via plain UVC (OpenCV), no ZED SDK/GPU needed., cv2.VideoCapture(int) enumerates devices per-backend, and on Linux the     V4L2/ (+2 more)
...
Cohesion: 0.18
Nodes (15): parse_args(), OcTree — sample a TUM-FACADE point cloud into voxels and visualize it.  Loads, Occupied octree leaves at depth d == voxelizer voxels at the matching size., selftest(), build_octree(), _cube_root(), leaf_voxels(), level_counts() (+7 more)

### Community 19 - "__init__.py"
Cohesion: 0.21
Nodes (8): _is_cached(), MaterialClassifier, Zero-shot material classification with CLIP.  Uses openai/clip-vit-base-patch32, Best-effort check for a local snapshot, just to print an honest message     -- f, EmissivityRecord, EmissivityTable, Path, Tabulated emissivity values loaded from a CSV file.  The CSV columns are: materi
...
Cohesion: 0.18
Nodes (4): ndarray, ZED 2i left-eye RGB frames via the ZED SDK Python API (pyzed)., Return the next frame as an RGB HxWx3 uint8 array., ZedSource
...
Cohesion: 0.29
Nodes (6): Adding materials, Emissivity Calculation, Setup, Structure, Usage, ZED 2i camera
...
Cohesion: 0.50
Nodes (3): ndarray, Classify the material in the image.          Args:             image: RGB image, Image
...
## Knowledge Gaps
- **81 isolated node(s):** `Setup`, `Options`, `Shooting a good set`, `Options`, `Getting the board numbers right` (+76 more)
...
- **Why does `_merge_adjacent_rectangles()` connect `_merge_adjacent_rectangles` to `smoothing.py`, `__init__.py`, `viewer.py`?**
  _High betweenness centrality (0.007) - this node is a cross-community bridge._
- **Why does `_UnionFind` connect `_merge_adjacent_rectangles` to `smoothing.py`, `__init__.py`?**
