...
Cohesion: 0.18
Nodes (19): Provenance for one output rectangle, parallel to a SubSurface's `polygons`., RectMergeInfo, cube_origin(), filter_by_count(), _grid_from_index(), ndarray, Fast voxel sampling of a point cloud (numpy).  Each point is mapped to an inte, Keep only voxels with at least `min_count` points (a density filter).      Dro (+11 more)
...
Cohesion: 0.14
Nodes (10): ABC, FrameSource, _open_capture(), Frame sources: still image, webcam, and ZED 2i stereo camera.  All sources retur, ZED 2i single-eye RGB frames via plain UVC (OpenCV), no ZED SDK/GPU needed., ZED 2i left-eye RGB frames via the ZED SDK Python API (pyzed)., cv2.VideoCapture(int) enumerates devices per-backend, and on Linux the     V4L2/, WebcamSource (+2 more)
...
Cohesion: 0.14
Nodes (14): main(), principal_yaw(), Path, Flatten `grid` onto a single plane, preserving class zoning.      offset_method:, Write the planar surface as OpenStudio-friendly JSON (polygons + roles).      Sc, Dominant horizontal direction of the voxels, in degrees (0-180).      PCA on the, Rotate `grid`'s centers by -yaw_deg about their horizontal centroid.      Keeps, Inverse of _rotate_grid_horizontal's rotation, for (..., 2) xy arrays. (+6 more)
...
Cohesion: 0.33
Nodes (7): _load_surface(), Path, Map a PlanarSurface (or its JSON) to an OpenStudio .osm model.  Thin adapter ove, Write an OpenStudio .osm from a PlanarSurface or its exported JSON., to_osm(), PlanarSurface, SubSurface
...
### Community 60 - "_rotate_surface_back"
Cohesion: 0.50
Nodes (3): PlaneAnchor, A previously computed RANSAC plane, reusable to keep a surface put.      Pass th, Capture the plane a RANSAC-fitted surface was built on, or None (legacy fit / no
