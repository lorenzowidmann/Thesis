...
% (sezione 5b). Utile sia per scartare la coda del percorso (es. il ritorno
% che si sovrappone al corridoio di andata, generando doppioni nella mappa)
% sia per isolare un tratto e provare la loop closure solo li', senza
% indovinare una soglia spaziale: una coordinata X puo' essere attraversata
% piu' volte in modo imprevedibile (curve, avanti-indietro locale), il
% tempo trascorso no. Da tarare guardando "Durata odometria utilizzabile"
% (Sezione 3b) e il plot "Traiettoria" di un run precedente con
% useTimeCut = false.
useTimeCut      = true;
timeCutStartSec = 10;   % s, scarta i keyframe con tempo trascorso < questo (NaN = dall'inizio)
timeCutEndSec   = 180;   % s, scarta i keyframe con tempo trascorso > questo (NaN = fino alla fine)

% Classificazione topologica dei keyframe (nodo/linea), sezione 6d.
% Ispirato a Xu et al., "When-to-Loop: Enhanced Loop Closure for LiDAR SLAM
% in Urban Environments Based on Scan Context" (Micromachines 2024): gli
% incroci/T-junction sono piu' affidabili per la loop closure (rivisitabili
% da piu' direzioni), i tratti dritti sono la categoria a rischio di falsi
% positivi da struttura ripetitiva (porte, colonne a passo costante).
% Qui NON si fa la segmentazione ground/ostacolo del paper: si usa come
% proxy la rotazione cumulata (dAng, come in Sezione 5, ma tra keyframe
% consecutivi) su una finestra di keyframe adiacenti, per catturare svolte
% distribuite su piu' passi e non solo scatti singoli. nodeAngThreshold e'
% volutamente piu' alto di kfAngle: deve rappresentare una svolta vera
% cumulata, non il rumore che gia' da solo fa scattare un nuovo keyframe.
nodeAngWindow    = 3;    % keyframe di raggio della finestra (totale 2w+1)
nodeAngThreshold = 45;   % gradi, soglia cumulata sopra cui il keyframe e' "nodo"

% Downsampling applicato a ogni keyframe, serve sia a Scan Context che a ICP
kfVoxel    = 0.05;   % m

% Filtro outlier (statistico, pcdenoise): per ogni punto si guarda la
% distanza media dai suoi NumNeighbors vicini piu' prossimi; i punti la cui
% distanza media supera media + Threshold*deviazione_standard (calcolate su
% tutta la nuvola) vengono scartati. Applicato in due punti della pipeline:
%   - per keyframe, appena estratto (Sezione 6, aiuta anche i fit successivi:
%     pavimento 6b, normali dei muri 6c, e le corrispondenze ICP di Sezione 8);
%   - sulla mappa finale ricostruita (Sezione 11), dove compaiono anche
%     outlier "di giunzione" tra scansioni diverse che non esistono nel
%     singolo keyframe.
useOutlierFilterKF    = true;
outlierKFNumNeighbors = 8;      % punti vicini usati per la statistica
outlierKFStdFactor    = 2;    % soglia in deviazioni standard, piu' basso = piu' aggressivo
...
% Vincolo di quota Z sulla traiettoria, dallo stesso pavimento della 6b.
% A differenza dei 13 loop (Sezione 9 sotto), floorDist e' una misura
% LOCALE fresca ad ogni keyframe (non porta dentro la deriva accumulata):
% per questo corregge la deriva Z residua che i loop, gia' concordi con
% l'odometria entro 15 cm, non hanno margine per correggere (vedi nota
% sopra a "Applicazione dei vincoli di loop"). MISURATO su questo dataset
% (variante AC.m, stesso pavimento): std quota pre-ottim. 0.44 m -> 0.15 m
% post, escursione Z pose 1.82 m -> 0.69 m, con infoGravZ=5.
...
% Peso aggiuntivo per topologia nodo/linea (Sezione 6d), moltiplicativo su
% infoLoopEff sotto. MISURATO su questo dataset: 12 dei 13 loop accettati
% coinvolgono almeno un nodo, solo 77->109 e' linea-linea — i nodi sono un
% segnale di affidabilita' per i loop ACCETTATI, non un filtro sui falsi
% positivi (72->133 era nodo-nodo ed e' comunque un falso positivo, gia'
% scartato dal controllo di coerenza con l'odometria, non da questo peso).
% topoWMixed = 1 lascia invariato il comportamento su nodo-linea (la
% maggioranza dei 13); nodo-nodo pesa di piu', linea-linea di meno.
topoWNodeNode = 1.5;
topoWMixed    = 1.0;
topoWLineLine = 0.7;
...
% Salvataggio della mappa corretta (Sezione 13), su disco come .pcd.
% Il nome file e' generato al momento del salvataggio (Sezione 13) come
...
% Vedi nota su useTimeCut/timeCutStartSec/timeCutEndSec in Sezione 1. Va
% PRIMA dell'estrazione nuvole (Sezione 6): i keyframe scartati non vengono
% nemmeno letti dalla bag, ne' entrano in loop closure o pose graph. A
% differenza di un taglio su coordinata spaziale, il tempo trascorso e'
% monotono per costruzione: nessun rischio di attraversamenti multipli o
% ambigui. NaN su un estremo lascia quel lato aperto.
if useTimeCut
...
for k = 1:nKF
    ...
    % Filtro outlier (Sezione 1): dopo il downsampling, cosi' e' piu' veloce
    % e la densita' uniforme del voxel grid rende la statistica dei vicini
    % piu' stabile.
...
if useGravityAlign
    ...
    % nel frame BODY (kfClouds non cambia con la releveling: e' sempre
    % sensor-centric). Riusata in Sezione 9 per il vincolo di quota Z sulla
    % traiettoria, indipendente dal fit di normale qui sopra: e' il termine
    % noto dello stesso piano, non un fit separato.
    floorDist = nan(nKF, 1);
    for k = 1:nKF
...
%% 6d. Classificazione topologica dei keyframe (nodo/linea)
% Dopo 6b/6c, kfPoses e' nella forma finale: e' la geometria che usera'
% davvero la loop closure, quindi la classificazione va fatta qui, non
% prima. Vedi nota su nodeAngWindow/nodeAngThreshold in Sezione 1.
[isNodeKF, cumAngKF] = classifyKfNodes(kfPoses, nodeAngWindow, nodeAngThreshold);

fprintf('\n--- Classificazione topologica keyframe ---\n');
fprintf('Nodo (svolta):  %d su %d (%.0f%%)\n', nnz(isNodeKF), nKF, 100*nnz(isNodeKF)/nKF);
fprintf('Linea (dritto): %d su %d (%.0f%%)\n', nnz(~isNodeKF), nKF, 100*nnz(~isNodeKF)/nKF);

%% 6e. Correzione diretta della quota Z dal pavimento (PRIMA di Scan Context/ICP)
% floorDist (Sezione 6b) e' una misura locale, non affetta dalla deriva
% Z accumulata: qui si applica direttamente a kfPoses, non solo come arco
% nel pose graph (Sezione 9). Motivo: Arel = kfPoses(i).A \ kfPoses(j).A in
% Sezione 8 e' l'INIZIALE di ICP per i candidati di loop. Con la deriva Z
% grezza (fino a ~1.8 m su questo dataset) dentro quell'iniziale, ICP puo'
% finire fuori dal bacino di convergenza su candidati di per se' validi
% (vedi commento Sezione 8 sul "Tinit troppo sbagliato"). Correggere qui
% da' a ICP una stima di partenza migliore; il vincolo nel pose graph
% (Sezione 9) resta comunque utile per difendere questa quota durante
% l'ottimizzazione dei loop.
%
...
% prima da' lo stesso risultato numerico, con meno rischio di interferire
% con la reintegrazione di 6c.
...
% Classificazione nodo/linea per candidato (vedi Sezione 6d): non decide
% ancora nulla, serve solo a leggere i risultati ICP della Sezione 8
% separando i casi noti per tipo di keyframe.
topoLabel = ["linea" "nodo"];   % topoLabel(isNodeKF(k)+1) -> "linea"/"nodo"
fprintf('--- Candidati con classificazione topologica ---\n');
...
for c = 1:size(allCandidates, 1)
    ...
    % Etichetta nodo/linea del candidato (Sezione 6d), solo diagnostica: non
    % influenza ancora accettazione o pesi.
    topoTag = sprintf('[%s-%s]', topoLabel(isNodeKF(i)+1), topoLabel(isNodeKF(j)+1));

    if bestRmse > icpMaxRMSE
        fprintf('  loop SCARTATO  %d -> %d  (rmse %.3f m, sopra soglia)  %s\n', ...
    ...
    elseif transErr > loopMaxTransErr || rotErr > loopMaxRotErr
        fprintf(['  loop SCARTATO  %d -> %d  (rmse %.3f m ok, ma contraddice ' ...
            'l''odometria di %.1f m / %.1f deg: falso positivo)  %s\n'], ...
            i, j, bestRmse, transErr, rotErr, topoTag);
    else
        loopConstraints{end+1} = {i, j, bestTform, bestRmse};   %#ok<SAGROW>
        fprintf('  loop accettato %d -> %d  (rmse %.3f m, scarto %.2f m / %.1f deg)  %s\n', ...
            i, j, bestRmse, transErr, rotErr, topoTag);
    end
end
...
for c = 1:nLoops
    ...
    % Peso topologico (Sezione 6d): nodo-nodo piu' affidabile, linea-linea
    % meno, nodo-linea invariato rispetto al comportamento precedente.
...
if useGravityAlign && useGravityFactor
    % useGravityZ/floorFitRate/validFloor gia' calcolati in Sezione 6e,
    % stessa correzione gia' applicata direttamente a kfPoses li'. Qui si
    % ripete solo come arco nel pose graph, per difendere quella quota
    % durante l'ottimizzazione dei loop (l'ottimizzatore potrebbe altrimenti
    % rimuoverla in parte per soddisfare gli altri vincoli).
    if useGravityZ
        fprintf('Vincolo Z nel pose graph attivato (peso %g)\n', infoGravZ);
    else
        fprintf('Nessun vincolo Z nel pose graph (vedi Sezione 6e)\n');
    end
...
function [isNode, cumAng] = classifyKfNodes(kfPoses, winHalf, angThresh)
    % Proxy nodo/linea (Xu et al. 2024, adattato): dAng tra keyframe
    % consecutivi (stessa formula della Sezione 5, ma tra kfPoses invece
    % che tra frame grezzi), sommato in valore assoluto su una finestra di
    % raggio winHalf per catturare svolte distribuite su piu' passi. Somma
    % assoluta e non media/rotazione netta: una S-curve non deve annullarsi.
    n = numel(kfPoses);
    dAng = zeros(n, 1);   % dAng(k) = rotazione da kfPoses(k-1) a kfPoses(k)
    for k = 2:n
...
function v = buildInfoVectorAniso(wFree, wRP, wZ)
    ...
    % (Sezione 9) e' attivo per il keyframe corrente.
