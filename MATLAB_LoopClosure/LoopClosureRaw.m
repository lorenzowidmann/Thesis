...
% Pulizia finale della mappa: snap dei punti sui piani (sezione 14).
% Rifittura RANSAC iterativa di pavimento/muri/soffitto sulla mappa gia'
% ottimizzata pcOpt, stesso approccio (RANSAC iterativo + dedupe dei
% frammenti + vincolo di parallelismo/ortogonalita' tra i muri) di
% MATLAB_LoopClosure/PlaneFittingAttempt/fit_planes.py: leggere quel file
% per i dettagli, qui e' la stessa logica riscritta in MATLAB.
useSnapPlanes = true;

snapMaxDistance  = 0.05;   % m, soglia inlier RANSAC (pcfitplane)
snapMaxNumTrials = 1000;   % tentativi RANSAC per singolo fit (pcfitplane 'MaxNumTrials')
snapMinInliers   = 2000;   % punti minimi per accettare un piano, sotto ci si ferma
snapMaxPlanes    = 20;     % numero massimo di piani da cercare (cap di sicurezza)
snapMaxAttempts  = 0;      % tentativi totali (accettati+scartati); 0 = snapMaxPlanes*5

% Pavimento e soffitto sono superfici enormi e coerenti: in RANSAC libero
% "vincono" ogni iterazione ed esauriscono l'intero budget snapMaxPlanes
% prima che si arrivi mai a un muro (osservato: 20/20 piani tutti
% floor_ceiling). Si cercano quindi prima loro con un fit VINCOLATO alla
% verticale (pcfitplane con ReferenceVector/MaxAngularDistance), fino a un
% tetto separato, cosi' i muri hanno un budget garantito sul resto della
% nuvola invece di competere sempre a svantaggio.
snapMaxHorizPlanes = 6;    % tetto ai piani pavimento/soffitto nella prima fase

% Un piano accettato ma non allineato alla struttura (es. un mobile, non
% un muro) viene scartato come rumore invece di essere usato per lo snap.
snapMaxTiltDeg = 15;    % gradi, scarto max dalla verticale/orizzontale prima di scartare come rumore (0 = disattiva)
snapWallMaxNz  = 0.5;   % |nz| sotto cui una normale e' di muro, sopra e' pavimento/soffitto

% Dedupe: fonde frammenti della stessa superficie fisica (normali
% pressoche' parallele, quasi complanari) invece di trattarli come piani
% distinti.
snapDedupeNormalDeg = 10;    % gradi, angolo massimo tra normali per la stessa superficie
snapDedupeOffsetM   = 0.15;  % m, distanza punto-piano massima per la stessa superficie (riusata anche nel collasso post-ortogonalizzazione)

% Vincolo di parallelismo/ortogonalita' tra i muri. pcfitplane assegna il
% segno della normale in modo arbitrario, quindi due muri opposti possono
% risultare con normali quasi UGUALI invece che opposte: il confronto va
% fatto come tra rette (valore assoluto del prodotto scalare), non tra
% vettori, altrimenti l'accoppiamento fallisce sempre.
snapParallelMaxAngleDeg = 45;    % gradi, scarto massimo dal parallelismo per accoppiare due muri opposti
snapParallelMinSepM     = 0.3;   % m, separazione minima perche' due muri paralleli siano lati opposti (sotto, sono frammenti dello stesso muro)

% Soft-snap: ogni punto entro snapAssignDistance dal piano piu' vicino
% viene spostato PARZIALMENTE verso quel piano, non proiettato in modo
% rigido: i muri sbavati su piu' registrazioni vicine si restringono senza
% perdere del tutto la variazione naturale della nuvola.
snapAssignDistance = 0.10;   % m, distanza massima punto-piano per l'assegnazione
snapBlendFactor    = 0.7;    % 0 = punto invariato, 1 = proiezione rigida sul piano
...
%% 14. Pulizia mappa: snap dei punti sui piani
% Rifinitura geometrica finale di pcOpt: fitta iterativamente pavimento,
% muri e soffitto con RANSAC (pcfitplane), li ripulisce (dedupe dei
% frammenti, vincolo di parallelismo/ortogonalita' tra i muri) e poi
% sposta PARZIALMENTE (soft-snap, non proiezione rigida) ogni punto vicino
% a un piano verso quel piano. Stessa logica di
% MATLAB_LoopClosure/PlaneFittingAttempt/fit_planes.py, riscritta qui in
% MATLAB; quel file resta il riferimento per il dettaglio di ogni passo.
if useSnapPlanes
    fprintf('\n--- Pulizia mappa: fit piani su pcOpt ---\n');

    workCloud = pcOpt;
    planes = struct('id', {}, 'normal', {}, 'd', {}, 'centroid', {}, ...
        'inlierCount', {}, 'orientation', {});

    if snapMaxAttempts <= 0
        snapMaxAttemptsEff = snapMaxPlanes * 5;
    else
        snapMaxAttemptsEff = snapMaxAttempts;
    end

    attempt = 0;

    % Fase 1: pavimento/soffitto, fit VINCOLATO alla verticale (entro
    % snapMaxTiltDeg da [0 0 1]) fino a snapMaxHorizPlanes. Il vincolo
    % impedisce che RANSAC continui a "trovare" pavimento/soffitto anche
    % dopo che i pezzi grossi sono gia' stati rimossi (frammenti residui a
    % quote diverse per rumore di registrazione), lasciando spazio ai muri.
    horizFound = 0;
    while horizFound < snapMaxHorizPlanes && numel(planes) < snapMaxPlanes && attempt < snapMaxAttemptsEff
        if workCloud.Count < max(50, snapMinInliers)
            fprintf('  %d punti rimasti, fermo (fase orizzontale).\n', workCloud.Count);
            break
        end
        try
            [model, inlierIdx, outlierIdx] = pcfitplane(workCloud, snapMaxDistance, ...
                [0 0 1], snapMaxTiltDeg, 'MaxNumTrials', snapMaxNumTrials);
        catch ME
            fprintf('  pcfitplane (orizzontale) fallito: %s\n', ME.message);
            break
        end
        attempt = attempt + 1;
        if numel(inlierIdx) < snapMinInliers
            fprintf(['  fase orizzontale: miglior fit %d inlier < snapMinInliers %d, ' ...
                'passo ai muri.\n'], numel(inlierIdx), snapMinInliers);
            break
        end

        n = double(model.Normal(:))';
        n = n / norm(n);
        inlierPts = workCloud.Location(inlierIdx, :);
        centroid  = mean(double(inlierPts), 1);

        idx = numel(planes) + 1;
        planes(idx).id          = idx;
        planes(idx).normal      = n;
        planes(idx).d           = -dot(n, centroid);
        planes(idx).centroid    = centroid;
        planes(idx).inlierCount = numel(inlierIdx);
        planes(idx).orientation = 'floor_ceiling';
        horizFound = horizFound + 1;

        fprintf('  piano %d: %d inlier, normale [%.3f %.3f %.3f], floor_ceiling, tilt %.1f deg\n', ...
            idx, numel(inlierIdx), n(1), n(2), n(3), tiltFromStructureDeg(n));

        workCloud = select(workCloud, outlierIdx);
    end
    fprintf('Fase orizzontale: %d piani pavimento/soffitto (tetto %d)\n', ...
        horizFound, snapMaxHorizPlanes);

    % Fase 2: muri, fit LIBERO sul resto della nuvola (pavimento/soffitto
    % gia' rimossi sopra, quindi qui i muri sono le superfici piu' grandi
    % rimaste e RANSAC li trova senza doverli spuntare contro l'orizzontale).
    while numel(planes) < snapMaxPlanes && attempt < snapMaxAttemptsEff
        if workCloud.Count < max(50, snapMinInliers)
            fprintf('  %d punti rimasti, fermo (fase muri).\n', workCloud.Count);
            break
        end

        try
            [model, inlierIdx, outlierIdx] = pcfitplane(workCloud, snapMaxDistance, ...
                'MaxNumTrials', snapMaxNumTrials);
        catch ME
            fprintf('  pcfitplane (muri) fallito: %s\n', ME.message);
            break
        end
        attempt = attempt + 1;

        if numel(inlierIdx) < snapMinInliers
            fprintf('  fase muri: miglior fit %d inlier < snapMinInliers %d, fermo.\n', ...
                numel(inlierIdx), snapMinInliers);
            break
        end
        ...
        tilt = tiltFromStructureDeg(n);
        if snapMaxTiltDeg > 0 && tilt > snapMaxTiltDeg
            fprintf(['  scarto: piano da %d inlier inclinato %.1f deg fuori asse ' ...
                '(> snapMaxTiltDeg %.1f), rimosso come rumore\n'], ...
                numel(inlierIdx), tilt, snapMaxTiltDeg);
            workCloud = select(workCloud, outlierIdx);
            continue
        end

        % Qui il fit e' libero: se capita ancora un residuo orizzontale
        % (pavimento/soffitto oltre il tetto della fase 1) resta comunque
        % classificato correttamente, non forzato a "wall".
        if abs(n(3)) > snapWallMaxNz
            orientation = 'floor_ceiling';
        else
            orientation = 'wall';
        end
        ...
        planes(idx).centroid    = centroid;
        planes(idx).inlierCount = numel(inlierIdx);
        planes(idx).orientation = orientation;

        fprintf('  piano %d: %d inlier, normale [%.3f %.3f %.3f], %s, tilt %.1f deg\n', ...
            idx, numel(inlierIdx), n(1), n(2), n(3), orientation, tilt);

        workCloud = select(workCloud, outlierIdx);
    end

    fprintf('%d piani estratti, %d punti non assegnati\n', numel(planes), workCloud.Count);

    fprintf('Dedupe frammenti...\n');
    planes = dedupePlanes(planes, snapDedupeNormalDeg, snapDedupeOffsetM);
    planes = renumberPlanes(planes);

    zRange = [min(pcOpt.Location(:,3)), max(pcOpt.Location(:,3))];
    planes = classifyHorizontal(planes, zRange);

    fprintf('Vincolo di parallelismo tra i muri...\n');
    planes = enforceParallelWalls(planes, snapParallelMaxAngleDeg, snapParallelMinSepM);

    fprintf('Vincolo di ortogonalita'' (muri <-> soffitto/pavimento)...\n');
    planes = enforceOrthogonalPlanes(planes);

    fprintf('Collasso lati duplicati...\n');
    planes = collapseDuplicateSides(planes, snapDedupeOffsetM);
    planes = renumberPlanes(planes);

    nPlanesFinal = numel(planes);
    fprintf('\n--- %d piani finali (muri + pavimento/soffitto) ---\n', nPlanesFinal);

    if nPlanesFinal == 0
        warning(['Nessun piano trovato: snap saltato. pcOpt salvato invariato ' ...
            'nella sezione 13; nessun file *_snapped.pcd scritto.']);
    else
        % Assegnazione punto -> piano piu' vicino (se entro snapAssignDistance),
        % vettorizzata su tutta la nuvola: un'iterazione per piano, non per punto.
        loc = double(pcOpt.Location);
        nPts = size(loc, 1);
        bestDist = inf(nPts, 1);
        bestIdx  = zeros(nPts, 1);
        bestSigned = zeros(nPts, 1);
        normalsMat = vertcat(planes.normal);

        for p = 1:nPlanesFinal
            signedDist = loc * planes(p).normal' + planes(p).d;
            absDist = abs(signedDist);
            mask = absDist < bestDist;
            bestDist(mask)   = absDist(mask);
            bestIdx(mask)    = p;
            bestSigned(mask) = signedDist(mask);
        end
        assigned = bestDist < snapAssignDistance;

        spreadBefore = zeros(1, nPlanesFinal);
        nAssignedPerPlane = zeros(1, nPlanesFinal);
        for p = 1:nPlanesFinal
            m = assigned & (bestIdx == p);
            nAssignedPerPlane(p) = nnz(m);
            if nAssignedPerPlane(p) > 1
                spreadBefore(p) = std(bestSigned(m));
            end
        end

        % Soft-snap: sposta ogni punto assegnato di snapBlendFactor lungo la
        % normale verso il piano, invece di proiettarlo esattamente sopra
        % (blend 1.0). Solo la componente perpendicolare al piano si
        % accorcia, la posizione nel piano (variazione naturale) resta.
        locNew = loc;
        aIdx = find(assigned);
        locNew(aIdx, :) = loc(aIdx, :) - snapBlendFactor * bestSigned(aIdx) .* normalsMat(bestIdx(aIdx), :);

        spreadAfter = zeros(1, nPlanesFinal);
        for p = 1:nPlanesFinal
            m = assigned & (bestIdx == p);
            if nAssignedPerPlane(p) > 1
                d = locNew(m, :) * planes(p).normal' + planes(p).d;
                spreadAfter(p) = std(d);
            end
        end

        for p = 1:nPlanesFinal
            fprintf(['  piano %2d: %-13s %7d inlier RANSAC, %7d punti mappa agganciati, ' ...
                'dispersione %.4f -> %.4f m\n'], ...
                planes(p).id, planes(p).orientation, planes(p).inlierCount, ...
                nAssignedPerPlane(p), spreadBefore(p), spreadAfter(p));
        end
        fprintf('Punti mappa: %d totali, %d agganciati a un piano (%.1f%%), blend %.2f\n', ...
            nPts, nnz(assigned), 100*nnz(assigned)/nPts, snapBlendFactor);

        pcSnapped = pointCloud(locNew);

        outFileSnapped = fullfile(fileparts(bagPath), 'loop_closed_map_snapped.pcd');
        pcwrite(pcSnapped, outFileSnapped, 'Encoding', 'binary');
        fprintf('Mappa con snap sui piani salvata in:\n  %s\n', outFileSnapped);
    end
end
...
function t = tiltFromStructureDeg(n)
    % Angolo (gradi) di n dall'orientamento Manhattan piu' vicino: 0 = normale
    % esattamente verticale (pavimento/soffitto), 90 = esattamente
    % orizzontale (muro). Vedi fit_planes.py::tilt_from_structure_deg.
    n = n / norm(n);
    angToVert = rad2deg(acos(min(1, abs(n(3)))));
    t = min(angToVert, abs(90 - angToVert));
end

function planesOut = renumberPlanes(planesIn)
    % Riassegna planesIn(i).id in ordine 1..N, cosi' i log successivi
    % mostrano id consecutivi anche dopo un filtraggio.
    planesOut = planesIn;
    for i = 1:numel(planesOut)
        planesOut(i).id = i;
    end
end

function planesOut = dedupePlanes(planesIn, normalDegThresh, offsetThresh)
    % Un pavimento/muro/soffitto reale viene spesso fittato come piu'
    % frammenti disgiunti (clutter, occlusioni) invece che come un piano
    % solo. Raggruppa i piani che sono di fatto la stessa superficie fisica
    % -- normali quasi parallele, quasi complanari -- e tiene solo il piu'
    % grande (per inlierCount) di ogni gruppo. Vedi fit_planes.py::dedupe_planes.
    %
    % L'angolo si confronta con abs(dot(...)): il segno della normale di
    % pcfitplane e' arbitrario, quindi due fit della stessa superficie
    % possono avere normali opposte pur essendo lo stesso piano fisico. La
    % complanarita' si verifica con la distanza punto-piano del centroide
    % di un frammento sul piano dell'altro (il minimo delle due direzioni):
    % |n.x + d| non dipende dal segno di (n, d), quindi non serve
    % canonicalizzare nulla per questo confronto.
    n = numel(planesIn);
    if n == 0
        planesOut = planesIn;
        return
    end
    [~, order] = sort([planesIn.inlierCount], 'descend');
    used = false(1, n);
    keptIdx = [];
    for oi = 1:n
        i = order(oi);
        ...
        used(i) = true;
        keptIdx(end+1) = i; %#ok<AGROW>
        ni = planesIn(i).normal; di = planesIn(i).d;
        dropped = [];
        for oj = 1:n
            j = order(oj);
            if used(j), continue; end
            nj = planesIn(j).normal; dj = planesIn(j).d;
            ang = rad2deg(acos(min(1, abs(dot(ni, nj)))));
            distJonI = abs(dot(ni, planesIn(j).centroid) + di);
            distIonJ = abs(dot(nj, planesIn(i).centroid) + dj);
            if ang < normalDegThresh && min(distJonI, distIonJ) < offsetThresh
                used(j) = true;
                dropped(end+1) = j; %#ok<AGROW>
            end
        end
        if ~isempty(dropped)
            fprintf('  dedupe: piano %d (%d inlier) assorbe %s come stessa superficie\n', ...
                planesIn(i).id, planesIn(i).inlierCount, mat2str([planesIn(dropped).id]));
        end
    end
    keptIdx = sort(keptIdx);
    planesOut = planesIn(keptIdx);
    fprintf('  dedupe: %d -> %d piani\n', n, numel(planesOut));
end

function planesOut = classifyHorizontal(planesIn, zRange)
    % Divide l'orientamento generico "floor_ceiling" in "floor"/"roof" in
    % base a quale estremo di quota (zRange = [zmin zmax] della nuvola) e'
    % piu' vicino al centroide del piano -- NON al segno della normale, che
    % pcfitplane assegna in modo arbitrario e non e' affidabile per capire
    % se un piano orizzontale e' sopra o sotto. Vedi
    % fit_planes.py::classify_horizontal.
    planesOut = planesIn;
    for i = 1:numel(planesOut)
        if ~strcmp(planesOut(i).orientation, 'floor_ceiling')
            continue
        end
        z = planesOut(i).centroid(3);
        if abs(z - zRange(1)) <= abs(z - zRange(2))
            planesOut(i).orientation = 'floor';
        else
            planesOut(i).orientation = 'roof';
        end
    end
end

function planesOut = enforceParallelWalls(planesIn, maxAngleDeg, minSepM)
    % Accoppia ogni muro con quello opposto (es. i due lati di un
    % corridoio) e li allinea su un asse condiviso, cosi' vengono
    % esattamente paralleli invece di portarsi dietro ognuno il proprio
    % rumore RANSAC. Vedi fit_planes.py::enforce_parallel_walls.
    %
    % L'angolo si confronta come tra RETTE (abs(dot(...))), non tra
    % vettori: il segno della normale di pcfitplane e' arbitrario, quindi
    % due muri opposti vengono spesso fittati con normali quasi UGUALI
    % invece che opposte. Confrontarle come vettori (angolo vicino a 180)
    % le farebbe scartare sempre come "non opposte".
    planesOut = planesIn;
    idxWalls = find(strcmp({planesOut.orientation}, 'wall'));
    unmatched = idxWalls;
    pairs = {};
    ...
    while ~isempty(unmatched)
        a = unmatched(1);
        unmatched(1) = [];
        na = planesOut(a).normal;

        bestJ = []; bestAngle = inf; bestCommon = [];
        for jj = 1:numel(unmatched)
            b = unmatched(jj);
            nb = planesOut(b).normal;
            ang = rad2deg(acos(min(1, abs(dot(na, nb)))));
            if ang < bestAngle
                if dot(na, nb) >= 0
                    aligned = nb;
                else
                ...
                common = na + aligned;
                if norm(common) > 1e-9
                    common = common / norm(common);
                    bestAngle = ang; bestJ = b; bestCommon = common;
                end
            end
        end

        if isempty(bestJ)
            fprintf('  parallel: muro %d (%d inlier) senza altro muro da accoppiare, lasciato com''e''\n', ...
                planesOut(a).id, planesOut(a).inlierCount);
            continue
        end
        if bestAngle >= maxAngleDeg
            fprintf(['  parallel: muro %d (%d inlier), candidato piu'' parallelo muro %d ' ...
                'a %.1f deg (> snapParallelMaxAngleDeg %.1f), lasciato com''e''\n'], ...
                planesOut(a).id, planesOut(a).inlierCount, planesOut(bestJ).id, bestAngle, maxAngleDeg);
            continue
        end

        sep = dot(bestCommon, planesOut(a).centroid - planesOut(bestJ).centroid);
        if abs(sep) < minSepM
            fprintf(['  parallel: muro %d e muro %d paralleli ma solo %.2f m separati ' ...
                '(< snapParallelMinSepM %.2f) -- frammenti dello stesso muro, lasciati com''e''\n'], ...
                planesOut(a).id, planesOut(bestJ).id, abs(sep), minSepM);
            continue
        end

        unmatched(unmatched == bestJ) = [];
        pairs{end+1} = struct('a', a, 'b', bestJ, 'angle', bestAngle, ...
            'common', bestCommon, 'sep', sep); %#ok<AGROW>
    end

    for k = 1:numel(pairs)
        pr = pairs{k};
        % Convenzione normali verso l'esterno: sep = (centroide_a -
        % centroide_b) . asse_comune, quindi a sta dal lato +asse_comune
        % quando sep > 0. I due segni restano sempre opposti, il che evita
        % che la coppia venga poi ri-fusa in un solo piano dal collasso.
        if pr.sep > 0, signA = 1; else, signA = -1; end
        idxs  = [pr.a, pr.b];
        signs = [signA, -signA];
        for m = 1:2
            p = idxs(m);
            nOut = pr.common * signs(m);
            planesOut(p).normal = nOut;
            planesOut(p).d = -dot(nOut, planesOut(p).centroid);
        end
        fprintf(['  parallel: muri %d e %d, %.1f deg da parallelo, %.2f m di distanza ' ...
            '-> asse comune, versi opposti\n'], ...
            planesOut(pr.a).id, planesOut(pr.b).id, pr.angle, abs(pr.sep));
    end
end

function [up, e1, e2] = buildOrthogonalFrame(planesIn)
    % Deriva un sistema d'assi ortonormale dai piani gia' fittati -- up piu'
    % due direzioni orizzontali a 90 gradi -- invece di assumere gli assi
    % del mondo: robusto anche se la struttura e' ruotata (yaw) rispetto al
    % frame mappa. Vedi fit_planes.py::build_orthogonal_frame.
    idxUp = find(strcmp({planesIn.orientation}, 'roof') | strcmp({planesIn.orientation}, 'floor'));
    if ~isempty(idxUp)
        upAcc = [0 0 0];
        for k = idxUp
            nk = planesIn(k).normal;
            if nk(3) < 0, nk = -nk; end
            upAcc = upAcc + nk * planesIn(k).inlierCount;
        end
        up = upAcc / norm(upAcc);
    else
        up = [0 0 1];
    ...
    idxWalls = find(strcmp({planesIn.orientation}, 'wall'));
    if isempty(idxWalls)
        e1 = []; e2 = [];
        return
    end
    [~, bi] = max([planesIn(idxWalls).inlierCount]);
    nRef = planesIn(idxWalls(bi)).normal;
    e1 = nRef - dot(nRef, up) * up;
    if norm(e1) < 1e-6
        e1 = []; e2 = [];
        return
    end
...
function planesOut = enforceOrthogonalPlanes(planesIn)
    % Aggancia ogni piano al sistema d'assi ortogonale della struttura:
    % pavimento/soffitto su +-up, muri su +-e1/+-e2. Due muri gia' resi
    % paralleli da enforceParallelWalls finiscono sullo stesso asse e
    % restano paralleli; un muro sull'altro asse (es. il fondo di un
    % corridoio) risulta esattamente perpendicolare a entrambi, e ogni muro
    % esattamente perpendicolare al soffitto/pavimento. Vedi
    % fit_planes.py::enforce_orthogonal_planes.
    %
    % floor/roof NON scelgono l'asse per prodotto scalare con +-up: il
    % segno della normale di pcfitplane e' arbitrario, quindi un pavimento e
    % un soffitto possono avere entrambi normale grezza ~+Z (nulla lega
    % l'una o l'altra a "verso l'esterno"). Usando il prodotto scalare
    % finirebbero sullo stesso verso e collapseDuplicateSides ne terrebbe
    % uno solo in base al numero di inlier, non affidabile (una scansione
    % rivolta verso il basso puo' vedere piu' pavimento che soffitto).
    % L'etichetta orientation di classifyHorizontal (basata sulla quota, non
    % sul segno) e' affidabile, quindi qui si usa direttamente.
    [up, e1, e2] = buildOrthogonalFrame(planesIn);
    planesOut = planesIn;
    if isempty(e1)
        fprintf('  orthogonal: nessun muro per ancorare una direzione orizzontale, salto\n');
        return
    end
    wallAxes = [e1; -e1; e2; -e2];
    for i = 1:numel(planesOut)
        switch planesOut(i).orientation
            case 'roof'
                best = up;
            case 'floor'
                best = -up;
            otherwise
                dots = wallAxes * planesOut(i).normal';
                [~, bi] = max(dots);
                best = wallAxes(bi, :);
        end
        planesOut(i).normal = best;
        planesOut(i).d = -dot(best, planesOut(i).centroid);
    end
    fprintf(['  orthogonal: %d piani agganciati al sistema d''assi ' ...
        '(up=[%.3f %.3f %.3f], assi muri [%.3f %.3f %.3f] / [%.3f %.3f %.3f])\n'], ...
        numel(planesOut), up, e1, e2);
end

function planesOut = collapseDuplicateSides(planesIn, offsetThresh)
    % Significativo solo subito dopo enforceOrthogonalPlanes: la normale di
    % ogni piano e' allora esattamente una delle 6 direzioni canoniche
    % della struttura, quindi frammenti della stessa superficie fisica che
    % dedupePlanes non aveva unito (es. perche' non erano stati accoppiati
    % da enforceParallelWalls) ora condividono la STESSA normale e possono
    % essere raggruppati senza perdite. Raggruppa per direzione (esatta) E
    % per offset: superfici parallele a offset diverso (es. i due lati di
    % un corridoio) restano piani distinti, solo i frammenti quasi
    % complanari vengono fusi. Vedi fit_planes.py::collapse_duplicate_sides.
    keysAll = round(vertcat(planesIn.normal), 6);
    [uniqueKeys, ~, grpIdx] = unique(keysAll, 'rows');
    keptIdx = [];
    for g = 1:size(uniqueKeys, 1)
        members = find(grpIdx == g)';
        [~, ord] = sort([planesIn(members).inlierCount], 'descend');
        members = members(ord);

        buckets = {};
        for m = members
            placed = false;
            for bIdx = 1:numel(buckets)
                if abs(planesIn(m).d - planesIn(buckets{bIdx}(1)).d) < offsetThresh
                    buckets{bIdx}(end+1) = m; %#ok<AGROW>
                    placed = true;
                    break
                end
            end
            if ~placed
                buckets{end+1} = m; %#ok<AGROW>
            end
        end

        for bIdx = 1:numel(buckets)
            b = buckets{bIdx};
            keptIdx(end+1) = b(1); %#ok<AGROW>
            if numel(b) > 1
                fprintf(['  collapse: direzione [%.2f %.2f %.2f] offset %.2f, tenuto piano %d ' ...
                    '(%d inlier), scartati %s come frammenti ridondanti\n'], ...
                    uniqueKeys(g,:), planesIn(b(1)).d, planesIn(b(1)).id, ...
                    planesIn(b(1)).inlierCount, mat2str([planesIn(b(2:end)).id]));
            end
        end
    end
    keptIdx = sort(keptIdx);
    planesOut = planesIn(keptIdx);
    fprintf('  collapse: %d -> %d piani\n', numel(planesIn), numel(planesOut));
end
